...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (password?.trim()?.length < 6)
    throw new ApiError(400, "password should be greater than 6 digits");

  const isEmailExist: IUser | null = await User.findOne({
    $or: [{ email }, { username }],
  });
  ...
  return res
    ...
    .json(
      new ApiResponse<IUserResponse>(201, data, "user registered successfully"),
    );
});
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  return res
    ...
    .json(new ApiResponse<IUserResponse>(200, data, "login successful"));
});
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  if (!username) throw new ApiError(500, "wrong web token");
  ...
  return res
    ...
    .json(
      new ApiResponse<{ accessToken: string }>(200, {
